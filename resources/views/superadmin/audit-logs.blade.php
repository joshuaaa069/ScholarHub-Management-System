                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <form method="GET" class="flex flex-1 gap-3">
                        <div class="relative flex-1 max-w-sm">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by action or description..." class="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 focus:border-blue-500 rounded-xl text-xs outline-none transition-all">
                        </div>
                    ...
                    <form action="{{ route('superadmin.audit-logs.clear') }}" method="POST" onsubmit="return confirm('Clear ALL audit log entries? This cannot be undone.');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="inline-flex items-center gap-2 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-100 text-xs font-bold px-4 py-2.5 rounded-xl transition whitespace-nowrap">
                            ...
                            <tbody class="divide-y divide-slate-100 text-xs font-semibold">
                                @forelse($logs as $log)
                                    <tr class="hover:bg-slate-50/40 transition">
                                        ...
                                        <td class="py-4 px-6 text-slate-400">{{ $log->created_at->format('M d, Y g:i A') }}</td>
                                        <td class="py-4 px-6 text-right">
                                            <form action="{{ route('superadmin.audit-logs.destroy', $log->id) }}" method="POST" class="inline m-0" onsubmit="return confirm('Delete this log entry?');">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr><td colspan="6" class="py-12 text-center text-slate-400 font-medium">No audit log entries yet.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
