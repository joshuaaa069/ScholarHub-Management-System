        ...
        <div class="flex-1 flex flex-col min-w-0">
            <header
                ...
                <a href="{{ route('superadmin.reports.export') }}"
                    class="ml-auto inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2.5 rounded-xl transition shadow-sm shadow-blue-600/10">
            ...
            <main class="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
                    <div class="p-6 border-b border-slate-100">
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Applications and approvals per scholarship, system-wide
                        </p>
                    </div>
                            ...
                            <tbody class="divide-y divide-slate-100 text-xs font-semibold">
                                @forelse($scholarshipBreakdown as $s)
                                    <tr class="hover:bg-slate-50/40 transition">
                                        ...
                                        <td class="py-4 px-6 text-slate-500">{{ $s->slots_total - $s->slots_left }} /
                                            {{ $s->slots_total }}
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="py-12 text-center text-slate-400 font-medium">No scholarship
                                            programs to report on yet.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
