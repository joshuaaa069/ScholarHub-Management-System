                                ...
                                @forelse($students as $student)
                                    <tr class="hover:bg-slate-50/40 transition">
                                        ...
                                        <td class="py-4 px-6 font-mono font-bold text-slate-900">
                                            {{ $student->gpa ? number_format($student->gpa, 2) : 'N/A' }}</td>
                                        <td class="py-4 px-6">
                                            @if($scholarIds->contains($student->id))
                                                <span
                                                    class="px-2.5 py-1 rounded-lg text-[10px] font-extrabold uppercase bg-emerald-50 text-emerald-700 border border-emerald-100">Active
                                                    Scholar</span>
                                            @else
                                                <span
                                                    class="px-2.5 py-1 rounded-lg text-[10px] font-extrabold uppercase bg-slate-100 text-slate-500 border border-slate-200">Applicant</span>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="py-12 text-center text-slate-400 font-medium">No students
                                            found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
