...
<head>
    ...
    <title>Students - CKC ScholarHub</title>
        ...
        <div class="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
            <header class="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-6 lg:px-10 shrink-0">
                <div class="flex items-center space-x-4">
                    ...
                    <h1 class="text-lg font-extrabold text-slate-900">Students</h1>
                </div>
            ...
            <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-10 space-y-6">
                <form method="GET" class="flex gap-3">
                    <div class="relative flex-1 max-w-sm">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name, email, or student number..." class="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 focus:border-blue-500 rounded-xl text-xs outline-none transition-all">
                    </div>
                        ...
                        <table class="w-full text-left border-collapse min-w-[800px]">
                            <thead class="bg-slate-50/70 border-b border-slate-100 text-slate-400 text-[11px] font-bold uppercase tracking-wider">
                                <tr>
                                    ...
                                    <th class="py-4 px-6">Standing</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100 text-xs font-semibold">
                                @forelse($students as $student)
                                    <tr class="hover:bg-slate-50/40 transition">
                                        ...
                                        <td class="py-4 px-6 text-slate-600">{{ $student->course ?? '—' }}</td>
                                        <td class="py-4 px-6 text-slate-500">{{ $student->year_level ?? '—' }}</td>
                                        <td class="py-4 px-6 font-mono font-bold text-slate-900">{{ $student->gpa ? number_format($student->gpa, 2) : 'N/A' }}</td>
                                        <td class="py-4 px-6">
                                            @if($scholarIds->contains($student->id))
                                                <span class="px-2.5 py-1 rounded-lg text-[10px] font-extrabold uppercase bg-emerald-50 text-emerald-700 border border-emerald-100">Active Scholar</span>
                                            @else
                                                <span class="px-2.5 py-1 rounded-lg text-[10px] font-extrabold uppercase bg-slate-100 text-slate-500 border border-slate-200">Applicant</span>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="5" class="py-12 text-center text-slate-400 font-medium">No students found.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
