            ...
            <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-10 space-y-6">
                ...
                <form method="GET" class="flex flex-col sm:flex-row gap-3">
                    ...
                    <select name="status" onchange="this.form.submit()"
                        class="bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-600 outline-none">
                        @foreach(['All', 'Pending', 'Under Review', 'Approved', 'Needs Revision', 'Rejected'] as $opt)
                            <option value="{{ $opt }}" {{ request('status', 'All') === $opt ? 'selected' : '' }}>{{ $opt }}
                            </option>
                        @endforeach
                    </select>
                    <button type="submit"
                        class="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-5 py-2.5 rounded-xl transition">Search</button>
                </form>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse min-w-[800px]">
                            ...
                            <tbody class="divide-y divide-slate-100 text-xs font-semibold">
                                    ...
                                    <tr class="hover:bg-slate-50/40 transition">
                                        <td class="py-4 px-6">
                                            <div class="font-bold text-slate-900 text-sm">
                                                {{ $app->student->name ?? 'Unknown Student' }}</div>
                                            <div class="text-[11px] text-slate-400 mt-0.5">{{ $app->student->email ?? '' }}
                                            </div>
                                        </td>
                                        <td class="py-4 px-6 text-slate-600 font-bold">
                                            {{ $app->scholarship->title ?? 'N/A' }}</td>
                                        <td class="py-4 px-6">
                                            <span
                                                class="px-2.5 py-1 rounded-lg text-[10px] font-extrabold tracking-wide uppercase
                                        ...
                                        <td class="py-4 px-6 text-right">
                                            <div class="inline-flex items-center justify-end space-x-2">
                                                <form action="{{ route('scholarshipadmin.applications.action', $app->id) }}"
                                                    ...
                                                    <button type="submit"
                                                        class="bg-emerald-500 hover:bg-emerald-600 text-white text-[11px] font-bold px-3 py-1.5 rounded-lg transition">Approve</button>
                                                </form>
                                                <form action="{{ route('scholarshipadmin.applications.action', $app->id) }}"
                                                    method="POST" class="inline m-0">
                                                    @csrf<input type="hidden" name="action" value="Reject">
                                                    <button type="submit"
                                                        class="bg-rose-500 hover:bg-rose-600 text-white text-[11px] font-bold px-3 py-1.5 rounded-lg transition">Reject</button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="py-12 text-center text-slate-400 font-medium">No applications
                                            found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
